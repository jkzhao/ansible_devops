...
from ansible_devops.settings import MONGO_HOST,MONGO_PORT,BASE_DIR
import yaml

class Mongodb(object):
    """Mongobd的CURD"""

    def __init__(self, db='ansible', collection='tasklog'):
        self.client = pymongo.MongoClient(MONGO_HOST, int(MONGO_PORT))
        self.db = self.client[db] #具体数据库
        self.col = self.db[collection] #具体集合
...
class InsertAdhocLog(object):
    """记录日志"""

    def __init__(self, taskid):
        s_conf = yaml.load(open('%s/conf/taskdo.yml' %BASE_DIR))
        self.task_id = ""
        if taskid:
